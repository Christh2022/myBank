...
class BankCardsController extends AbstractController
{
    ...
    public function index(BankCardsRepository $repo): JsonResponse
    {
        ...
        return $this->json($cards, Response::HTTP_OK, [], ['groups' => ['bankcards:read']]);
    }
    ...
    public function new(Request $request, EntityManagerInterface $em, SerializerInterface $serializer, ValidatorInterface $validator): JsonResponse
    {
        ...
        try {
            ...
            return $this->json($bankCard, Response::HTTP_CREATED, [], ['groups' => ['bankcards:read']]);
        } catch (\Exception $e) {
    ...
    #[Route('/by-number/{number}', name: 'api_bankcards_by_number', methods: ['GET'])]
    public function getByNumber(string $number, BankCardsRepository $repo): JsonResponse
    {
        $user = $this->getUser();

        // Récupère la carte par numéro et utilisateur
        $bankCard = $repo->findOneBy([
            'number' => $number,
            'user' => $user, 
        ]);

        if (!$bankCard) {
            return $this->json(['error' => 'Card not found or access denied'], Response::HTTP_NOT_FOUND);
        }

        return $this->json($bankCard, Response::HTTP_OK, [], ['groups' => ['bankcards:read']]);
    }
}
